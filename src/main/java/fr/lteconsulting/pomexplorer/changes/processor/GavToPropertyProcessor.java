...
/**
 * Sometimes a GavChange consist in fact to change property values. This processor manages those cases
 */
	...
	protected void processChange(WorkingSession session, StringBuilder log, GavChange change, IChangeSet changeSet)
	{
		...
		if (Tools.isMavenVariable(depLoc.getUnresolvedGav().getVersion()))
		{
			...
			if ("project.version".equals(property))
			{
				...
				changeSet.addChange(new GavChange(projectLoc, new GAV(projectGav.getGroupId(), projectGav.getArtifactId(),
						change.getNewGav().getVersion())), change);
				return;
			}
			...
			if ("project.parent.version".equals(property))
			{
				...
				changeSet.addChange(
						new GavChange(projectLoc, new GAV(parentProjectGav.getGroupId(), parentProjectGav.getArtifactId(),
								change.getNewGav().getVersion())), change);
				return;
			}
			...
			{
				changeSet.addChange(new PropertyChange(new PropertyLocation(definitionProject, depLoc, property,
						definitionProject.getUnresolvedPom()
								.getProperties().getProperty(property)), change.getNewGav().getVersion()), change);
			}
		}
