...
public class PomAnalyzer
{
	...
	public void registerExternalDependency(WorkingSession session, Client client, StringBuilder log, GAV gav)
	{
		...
		try
		{
			resolvedArtifact = resolver.resolve(gav.toString()).withoutTransitivity()
	...
	private void processPom(File pomFile, WorkingSession session, Client client)
	{
		...
		if (resolved == null)
		{
			client.send("<span style='color:orange;'>cannot load pom " + unresolved.getGroupId() + ":"
					+ unresolved.getArtifactId() + ":" + unresolved.getVersion() + " (<i>" + pomFile.getAbsolutePath()
					+ "</i>)</span><br/>");
			return;
		}
	...
	private static MavenWorkingSession createMavenWorkingSession(WorkingSession workingSession)
	{
		try
		{
			...
			session = new AddScopedDependenciesTask(ScopeType.COMPILE, ScopeType.IMPORT, ScopeType.SYSTEM,
					ScopeType.RUNTIME).execute(session);
