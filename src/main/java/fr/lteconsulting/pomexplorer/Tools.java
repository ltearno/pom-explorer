...
public class Tools
{
	...
	public static Set<Location> getDirectDependenciesLocations(WorkingSession session, StringBuilder log, GAV gav)
	{
		...
		for (GAVRelation<Relation> relation : relations)
		{
			...
			{
				if (log != null)
					log.append("<span style='color:orange;'>Cannot find project for GAV " + updatedGav
							+ " which dependency should be modified ! skipping.</span>");
				continue;
			}
			...
			{
				if (log != null)
					log.append("<span style='color:red;'>Cannot find the location of dependency to " + relation.getTarget()
							+ " in this project " + updatedProject + "</span><br/>");
				continue;
			}
	...
	public static GavLocation findDependencyLocationInDependencies(WorkingSession session, Project project,
			GAV searchedDependency)
	{
		...
		// parent
		GavLocation locationInParent = findDependencyLocationInDependencies(session,
				session.projects().get(session.graph().parent(project.getGav())), searchedDependency);
